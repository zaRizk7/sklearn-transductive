...
def _mask_labels(estimator, y, train, test):
    intersect = np.intersect1d(train, test)

    is_clf = is_classifier(estimator)
    is_transductive = len(intersect) > 0
    y_masked = y.copy()
...
def cross_validate(
    ...
    *,
    ...
    if _routing_enabled():
        ...
        routed_params.estimator = Bunch(fit=params, predict=params)
...
# TODO (VALIDATE+TEST): Allow predict_params
# TODO (VALIDATE+TEST): Allow label masking when having intersecting labels
def _fit_and_score(
    estimator,
    X,
    y,
    *,
    ...
    predict_params_train = _check_method_params(X, params=predict_params, indices=train)
    ...
    y_masked, is_transductive = _mask_labels(estimator, y, train, test)
    ...
    *,
    ...
    predictions = parallel(
        delayed(_fit_and_predict)(
            ...
            routed_params.estimator.predict,
            method,
        )
...
# TODO (VALIDATE+TEST): Allow predict_params
def _fit_and_predict(estimator, X, y, train, test, fit_params, predict_params, method):
    ...
    predict_params = predict_params if predict_params is not None else {}
    predict_params = _check_method_params(X, params=predict_params, indices=test)

    y_masked = _mask_labels(estimator, y, train, test)
    ...
    for train, test in cv.split(X, y, **split_params):
        ...
        avg_score.append(
            scorer(
                ...
                X_test,
                y_test,
                predict_params=predict_params_test,
                **score_params_test,
            )
        )
...
def learning_curve(
    ...
    *,
    ...
    if _routing_enabled():
        ...
        routed_params = Bunch()
        routed_params.estimator = Bunch(fit=params, partial_fit=params, predict=params)
        routed_params.splitter = Bunch(split={"groups": groups})
        routed_params.scorer = Bunch(score={})
    ...
    for n_train_samples, partial_train in partitions:
        ...
        train_scores.append(
            _score(
                ...
                predict_params=predict_params_train,
                error_score=error_score,
            )
        )
